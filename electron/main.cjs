...
async function installLauncherUpdate() {
  ...
  const launchExe = fs.existsSync(currentExe)
    ? currentExe
    : fallbackLaunchExe
  const installDir = path.dirname(launchExe)
  ...
  const psScript = [
    ...
    `$targetVersion = '${escapePowerShellSingleQuoted(update.version || '')}'`,
    ...
    '  }',
    '}',
    '$initialStamp = if (Test-Path -LiteralPath $app) { (Get-Item -LiteralPath $app).LastWriteTimeUtc } else { [datetime]::MinValue }',
    '$initialVersion = Get-AppVersion $app',
    '$appResources = Join-Path (Split-Path -Parent $app) "resources\\app.asar"',
    '$initialResourcesStamp = if (Test-Path -LiteralPath $appResources) { (Get-Item -LiteralPath $appResources).LastWriteTimeUtc } else { [datetime]::MinValue }',
    'try {',
    ...
    "  Write-UpdateLog ('installer-start ' + ($installerArgs -join ' '))",
    "  Start-Process -FilePath $installer -ArgumentList $installerArgs -WindowStyle Hidden | Out-Null",
    '  $deadline = (Get-Date).AddMinutes(3)',
    '  do {',
    '    $appExists = Test-Path -LiteralPath $app',
    '    $appUnlocked = Test-FileUnlocked $app',
    '    $stampChanged = $false',
    '    $versionChanged = $false',
    '    $currentVersion = ""',
    '    $resourcesReady = $false',
    '    $resourcesStampChanged = $false',
    '    if ($appExists) {',
    '      $appItem = Get-Item -LiteralPath $app',
    '      $stampChanged = $appItem.LastWriteTimeUtc -gt $initialStamp',
    '      $currentVersion = Get-AppVersion $app',
    '      $versionChanged = (-not [string]::IsNullOrWhiteSpace($targetVersion)) -and ($currentVersion -eq $targetVersion) -and ($currentVersion -ne $initialVersion)',
    '    }',
    '    if (Test-Path -LiteralPath $appResources) {',
    '      $resourcesReady = $true',
    '      $resourcesStampChanged = (Get-Item -LiteralPath $appResources).LastWriteTimeUtc -gt $initialResourcesStamp',
    '    }',
    '    if ($appExists -and $appUnlocked -and $resourcesReady -and (($versionChanged -or $stampChanged) -or $resourcesStampChanged)) {',
    "      Write-UpdateLog ('update-ready version=' + $currentVersion + ' resourcesChanged=' + $resourcesStampChanged)",
    '      break',
    '    }',
    '    Start-Sleep -Milliseconds 700',
    '  } while ((Get-Date) -lt $deadline)',
    '  if ((Get-Date) -ge $deadline) { throw "installer-timeout" }',
