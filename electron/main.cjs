...
async function installLauncherUpdate() {
  ...
  const psScript = [
    ...
    '  $installerProcess = Start-Process -FilePath $installer -ArgumentList $installerArgs -WindowStyle Hidden -PassThru',
    '  if ($null -eq $installerProcess) { throw "installer-start-failed" }',
    ...
    '    if ((Test-Path -LiteralPath $app) -and (Test-FileUnlocked $app)) {',
    "      Write-UpdateLog 'app-ready'",
    '      break',
    '    }',
    '    Start-Sleep -Milliseconds 500',
    '  }',
    '  if (-not (Test-Path -LiteralPath $app)) { throw "app-not-found: $app" }',
    '  if (-not (Test-FileUnlocked $app)) { throw "app-still-locked" }',
    '  Start-Sleep -Milliseconds 650',
    "  Remove-Item Env:ELECTRON_RUN_AS_NODE -ErrorAction SilentlyContinue",
    "  [Environment]::SetEnvironmentVariable('ELECTRON_RUN_AS_NODE', $null, 'Process')",
    '  Start-Process -FilePath $app -WorkingDirectory (Split-Path -Parent $app) | Out-Null',
